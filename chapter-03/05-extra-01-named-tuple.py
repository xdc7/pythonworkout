"""
If you find tuples annoying because they use numeric indexes, you’re not alone! Re-implement this exercise using namedtuple objects, defined in the collections module. Many people like to use named tuples because they give the right balance between readability and efficiency
...
def printTuple(people):	
  for person in sorted(people, key=operator.itemgetter(1,0)):
    print ("{1:10} {0:10} {2:5.2f}".format(personTuple(*person).lastName,personTuple(*person).firstName,personTuple(*person).duration))
